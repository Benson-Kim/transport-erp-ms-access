...
Public Function AuthenticateUser(ByVal strUsername As String, _
    ...
    If Len(strUsername) = 0 Or Len(strPassword) = 0 Then GoTo LoginFailed
    ...
    If rs.EOF Then GoTo LoginFailed
    ...
    If Not blnIsActive Then
        MsgBox "This account has been deactivated.", vbCritical, modGlobals.APP_NAME
        GoTo LoginFailed
    End If
    ...
    If Not modCrypto.VerifyPassword(strPassword, Nz(strStoredHash, "")) Then
        GoTo LoginFailed
    End If
    ...
    If blnExpired Then
        ...
        dtPasswordSet = Nz(rs!PasswordSetDate, #1/1/1900#)
    ...
    rs.Close
    Set rs = db.OpenRecordset("Users", dbOpenDynaset)   ' Need dynaset to edit
...
LoginFailed:
    ...
    If lngUserID > 0 Then
        ...
        If Not rs.NoMatch Then
            ...
            If Not IsNull(dtLockoutUntil) Then rs!LockoutUntil = dtLockoutUntil
            rs.Update
        End If
    End If
...
ErrorHandler:
    modUtilities.LogError MODULE_NAME & ".AuthenticateUser", Err.number, Err.Description, _
...
ErrorHandler:
    modUtilities.LogError MODULE_NAME & ".ChangePassword", Err.number, Err.Description
...
ErrorHandler:
    modUtilities.LogError MODULE_NAME & ".CreateUser", Err.number, Err.Description
