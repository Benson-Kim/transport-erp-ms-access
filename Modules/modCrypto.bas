...
' SECURITY  : Uses SHA-256 with random salt for password hashing
' NOTES     :
'   - Requires Windows Vista or later
'   - Uses Base64 encoding for storage
'   - Depends on modUtilities for error logging
'   - Uses ADODB.Stream for UTF-8 conversion
'   - Uses MSXML2.DOMDocument for Base64 encoding/decoding
'   - Ensure references to "Microsoft XML, v6.0" and "Microsoft ActiveX Data Objects 2.x Library" are set
...
Private Const BCRYPT_USE_SYSTEM_PREFERRED_RNG As Long = &H2  ' Flag for system RNG
...
Public Function HashPassword(ByVal strPassword As String) As String
    ...
    If GenerateSecureBytes(16, arrSalt) = False Then
        GoTo ErrorHandler
    End If
...
ErrorHandler:
    ...
    modUtilities.LogError MODULE_NAME & ".HashPassword", Err.number, Err.Description
    HashPassword = ""
    Resume CleanExit
End Function
...
Private Function StringToUTF8(ByVal strText As String) As Byte()
    ...
    If objStream Is Nothing Then GoTo ErrorHandler  ' Handle if missing
...
ErrorHandler:
    ...
    StringToUTF8 = arrBytes
    modUtilities.LogError MODULE_NAME & ".StringToUTF8", Err.number, Err.Description, "ADODB.Stream may be unavailable"
...
Private Function EncodeBase64(arrData() As Byte) As String
    ...
    If objXML Is Nothing Then GoTo ErrorHandler
...
ErrorHandler:
    EncodeBase64 = ""
    modUtilities.LogError MODULE_NAME & ".EncodeBase64", Err.number, Err.Description, "MSXML2.DOMDocument may be unavailable"
    Set objNode = Nothing
    Set objXML = Nothing
End Function
...
ErrorHandler:
    ...
    modUtilities.LogError MODULE_NAME & ".DecodeBase64", Err.number, Err.Description, "MSXML2.DOMDocument may be unavailable"
    Set objNode = Nothing
    Set objXML = Nothing
End Function

' NEW: Random Bytes Wrapper
Private Function GenerateSecureBytes(ByVal lngLength As Long, ByRef arrBuffer() As Byte) As Boolean
    On Error GoTo ErrorHandler

    Dim lResult As Long

    ' Use NULL handle for system-preferred RNG
    lResult = BCryptGenRandom(0, VarPtr(arrBuffer(0)), lngLength, BCRYPT_USE_SYSTEM_PREFERRED_RNG)

    If lResult <> 0 Then GoTo ErrorHandler

    GenerateSecureBytes = True

    Exit Function

ErrorHandler:
    GenerateSecureBytes = False
    modUtilities.LogError MODULE_NAME & ".GenerateSecureBytes", lResult, "BCryptGenRandom failed"
